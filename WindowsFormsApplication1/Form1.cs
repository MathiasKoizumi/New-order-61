...
namespace WindowsFormsApplication1
{
	public class Form1 : Form
	{
		...
		private void InitializeComponent()
		{
			...
			comboBox1.Items.AddRange(new object[9]
			{
			...
			comboBox1.Size = new System.Drawing.Size(251, 21);
		...
		public Form1()
		{
			...
			timer2 = new System.Timers.Timer(300000.0);
		...
		public void playMP3()
		{
			string[] array = new string[9]
			{
				...
				(string)comboBox1.Items[1],
				(string)comboBox1.Items[2],
				(string)comboBox1.Items[3],
				(string)comboBox1.Items[4],
				(string)comboBox1.Items[5],
				(string)comboBox1.Items[6],
				(string)comboBox1.Items[7],
				(string)comboBox1.Items[8]
			};
			for (int i = 0; i < 9; i++)
			{
				Stream manifestResourceStream = GetType().Assembly.GetManifestResourceStream("WindowsFormsApplication1.Resources.Music." + array[i]);
				if (!File.Exists(array[i]))
				{
					using (Stream output = File.Create(array[i]))
					{
			...
			{
				wplayer.URL = array[comboBox1.SelectedIndex];
		...
		public void FlashMob(int p)
		{
			...
			case 0:
			{
				...
				Stream manifestResourceStream2 = executingAssembly2.GetManifestResourceStream("WindowsFormsApplication1.Resources.Images.peace.png");
				Image image2 = Image.FromStream(manifestResourceStream2);
				if (junkie.Next(2) == 0)
				{
					gfx.DrawImage(image2, 500 - image2.Width / 2, 200 - image2.Width / 2, (float)((double)image2.Width * 2.5), (float)((double)image2.Height * 2.5));
				}
				break;
			}
			case 1:
			{
				Assembly executingAssembly = Assembly.GetExecutingAssembly();
				Stream manifestResourceStream = executingAssembly.GetManifestResourceStream("WindowsFormsApplication1.Resources.Images.skullandguns.png");
				Image image = Image.FromStream(manifestResourceStream);
				if (junkie.Next(2) == 0)
				{
					gfx.DrawImage(image, 600 - image.Width / 2, 500 - image.Width / 2, (float)((double)image.Width * 1.5), (float)((double)image.Height * 1.5));
				}
				break;
			}
			}
		...
		public void sayonaraCaptainHereComesTheRain()
		{
			if (junkie.Next(3) == 0)
			{
				...
				case 1:
					...
					break;
				case 2:
					manifestResourceStream = executingAssembly.GetManifestResourceStream("WindowsFormsApplication1.Resources.Images.scene3.jpg");
					image = Image.FromStream(manifestResourceStream);
					break;
				case 3:
		...
		private void juggleMeNotPleaseHornyBilbaoWorksFine()
		{
			for (int i = 0; i < 10; i++)
			{
				sayonaraCaptainHereComesTheRain();
				new Spaceship1(this);
			}
		}
		...
		public void WalkAroundSpaceman()
		{
			...
			for (int i = 0; i < junkie.Next(30); i++)
			{
				gfx.DrawImage(image, new Rectangle(new Point(num + (int)(300.0 * Math.Sin(junkie.Next(10000))), num2 + (int)(500.0 * Math.Sin(junkie.Next(10000)))), new Size(new Point((int)((double)image.Width * 0.3), (int)((double)image.Height * 0.3)))));
			}
		}
	}
}
